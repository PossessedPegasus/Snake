$(document).ready(function() { 
    ...
    //mENU
    var scren = 0; //Testing Arrea Change to 1 to start the game 
...
//plAY bOX
		ctx.globalAlpha = 0.5;
		ctx.fillStyle= "red";
		ctx.fillRect( w/2 - 120,  h-200, 240, 100)
		ctx.globalAlpha = 1
...
//oPTION bOX
	...
	ctx.font="10px Comic Sans MS"
 	ctx.fillText("("+ mx+","+Math.floor(my) + ")", 920, 20);
			...
			//For loop Border
			//Start at zero, go to the end of the canvas by 20 pixels per iteration
			ctx.fillStyle= "#33FF00"		
			...
			while(c < w){		//   H   w  ?
				ctx.fillRect(c, h - 17, 17, 408408);
				c += 23;
			}
		
		
			screenAni ++;
			if(screenAni > 15) screenAni = 5;
